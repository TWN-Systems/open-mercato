...
gantt
    ...
    section P0 — Immediate (v0.4.9 critical)
    NEW-01: vm sandbox escape (replace node:vm)       :crit, p0a, 2026-03-29, 1d
    F01: SSRF CALL_WEBHOOK (validateWebhookUrl)       :crit, p0b, 2026-03-29, 1d

    section P1 — This Sprint
    NEW-02: Remove chown sudoers from Dockerfile      :p1a, 2026-03-30, 1d
    A1: Content-Length check before arrayBuffer()     :p1b, 2026-03-30, 1d
    F08: Pin GitHub Actions to SHAs (supply chain)   :crit, p1c, 2026-03-30, 1d
    RC1: Quote TOCTOU — em.transactional + PESSIMISTIC_WRITE :p1d, 2026-03-31, 1d
    RC2: @Index on sessionToken + chat rate limit     :p1e, 2026-03-31, 1d
    F03: HTTP security headers in next.config.ts      :p1f, 2026-03-31, 1d

    section P2 — Next Sprint
    F17: Meilisearch 127.0.0.1 bind + firewall        :p2a, 2026-04-07, 1d
    F02: Integration credentials masking              :p2b, 2026-04-07, 3d
    F08: Add Dependabot for GitHub Actions            :p2c, 2026-04-08, 1d

    section P3 — Backlog
    A3/A5/A6: Pagination caps                         :p3a, 2026-04-14, 2d
    A7: SSE per-user connection limit                 :p3b, 2026-04-14, 2d
    A2/A4/A8: Rate limits (chat, workflow, reindex)  :p3c, 2026-04-16, 2d
    F12: Password minimum length env setting          :p3d, 2026-04-17, 1d
```
...
flowchart LR
    subgraph Current
        direction TB
        B["Browser\nCmd+K"] --> API["/api/chat"] --> OC["OpenCode\n:4096 (Docker)"] --> MCP["MCP Server\n:3001"] --> APP["Application APIs"]
    end

    subgraph Proposed BYOAI
        direction TB
        EXT["External AI Client\nClaude Desktop\nAny MCP client"] --> MCPH["MCP HTTP Server\nhttps://app.example.com/mcp\nAuthenticated via API key"] --> APP2["Application APIs\nSame ACL enforcement"]
    end

    Current -.->|"Eliminates\nvm sandbox\nOpenCode binary\nsupply chain dep"| Proposed BYOAI
```
